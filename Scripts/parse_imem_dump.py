...
def parse_hex(input_file, output_file):
    ...
    if(len(words) > int(0x18000/4)):

        for _ in range(int(0x18000/4), len(words)):
            if(words[-1] == "00000000"):
                words.pop()
            else:
    ...
    for word in words:
        if(addr % 16 == 0):
            tmp_lines.append("\n")
        tmp_lines.append(f"0x[{addr:04X}] : 0x[{word}] ")
        tmp_lines.append("\t")
        addr = addr + 4
...
if __name__ == "__main__":
    ...
    parse_hex("./dump/imem_dut_dump.hex", "./dump/imem_dut_dump.log")
    parse_hex("./dump/imem_gold_dump.hex", "./dump/imem_gold_dump.log")